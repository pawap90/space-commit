...
import TypeWriterEffectHelper from "../utils/TypeWriterEffectHelper.js";
export default class InitialScene extends Phaser.Scene {
  ...
  addComponents(fontFamily, titleSize, labelSize, smallLabelSize) {
    ...
    const startText = this.add.text(16, 0, "Press enter to start", {fontFamily, fontSize: titleSize, color: fontColor});
    startText?.setPosition(this.cameras.main.width / 2 - startText.width / 2, 200);
    const explorerLabel = this.add.text(16, 0, "", {fontFamily, fontSize: smallLabelSize, color: fontColor});
    explorerLabel.setPosition(400, 400);
    const username = this.add.text(16, 0, "", {fontFamily, fontSize: labelSize, color: fontColor});
    username.setPosition(explorerLabel.x, explorerLabel.y + explorerLabel.height);
    const experimentLabel = this.add.text(16, 0, "", {fontFamily, fontSize: smallLabelSize, color: fontColor});
    ...
    const experiment = this.add.text(16, 0, "", {fontFamily, fontSize: labelSize, color: fontColor});
    experiment.setPosition(experimentLabel.x, experimentLabel.y + experimentLabel.height);
    const messageLabel = this.add.text(16, 0, "", {fontFamily, fontSize: smallLabelSize, color: fontColor});
    messageLabel.setPosition(experiment.x, experiment.y + experiment.height * 2);
    const message = this.add.text(16, 0, "", {fontFamily, fontSize: labelSize, color: fontColor, wordWrap: {width: 700, useAdvancedWrap: true}});
